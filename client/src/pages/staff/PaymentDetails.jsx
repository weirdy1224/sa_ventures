  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: 0, color: 'var(--white)', fontSize: 20, fontWeight: 700 }}>Payment Records</h1>
        <p style={{ margin: 0, color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>All transaction history</p>
      </div>
            ...
            {loading ? Array(5).fill(0).map((_, i) => <tr key={i}><td colSpan={6}><div className="skeleton" style={{ height: 36 }} /></td></tr>)
              : payments.map(p => (
                <tr key={p._id}>
                  ...
                  <td style={{ fontSize: 12, fontFamily: 'monospace', color: 'rgba(255,255,255,0.4)' }}>{p.razorpayPaymentId || '—'}</td>
                  <td style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>{new Date(p.createdAt).toLocaleDateString('en-IN')}</td>
                </tr>
              ))}
          </tbody>
        </table>
