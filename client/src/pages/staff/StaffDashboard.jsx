  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: 0, color: 'var(--white)', fontSize: 20, fontWeight: 700 }}>Orders Dashboard</h1>
        <p style={{ margin: 0, color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>Manage and assign orders</p>
      </div>

      {/* Filter tabs */}
      <div style={{ display: 'flex', gap: 0, background: 'rgba(255,255,255,0.05)', borderRadius: 10, padding: 4, width: 'fit-content', marginBottom: 20, border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            {loading ? Array(6).fill(0).map((_, i) => <tr key={i}><td colSpan={7}><div className="skeleton" style={{ height: 36 }} /></td></tr>)
              : orders.map(o => (
                <tr key={o._id}>
                  ...
                  <td style={{ fontSize: 13 }}>{o.assignedTo?.name || <span style={{ color: 'rgba(255,255,255,0.3)' }}>Unassigned</span>}</td>
                  <td style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{o.expectedDelivery ? new Date(o.expectedDelivery).toLocaleDateString('en-IN') : '—'}</td>
                  <td>
                    <select onChange={e => handleAssign(o._id, e.target.value)} defaultValue="" style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, color: 'rgba(255,255,255,0.7)', padding: '5px 10px', fontSize: 12, cursor: 'pointer' }}>
