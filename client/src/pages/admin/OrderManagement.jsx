  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, color: 'var(--text-primary)', fontSize: 20, fontWeight: 700 }}>Order Management</h1>
          <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: 13 }}>Manage all customer orders</p>
        </div>
      </div>
            ...
            {loading ? (
              ...
              <tr key={o._id}>
                ...
                <td>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{o.customer?.email}</div>
                </td>
                <td style={{ fontWeight: 700 }}>₹{o.totalAmount?.toLocaleString('en-IN')}</td>
                <td><OrderStatusBadge status={o.status} /></td>
                <td><OrderStatusBadge status={o.paymentStatus} /></td>
                <td style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{new Date(o.createdAt).toLocaleDateString('en-IN')}</td>
                <td>
        ...
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setSelectedOrder(null); }}>
          <div className="modal modal-dark" style={{ maxWidth: 520 }}>
            <div className="modal-header">
              <h2 style={{ margin: 0, color: 'var(--text-primary)', fontSize: 17 }}>Order #{selectedOrder._id?.toString().slice(-8).toUpperCase()}</h2>
              <button onClick={() => setSelectedOrder(null)} style={{ background: 'var(--white)', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: 18 }}>✕</button>
            </div>
            <div className="modal-body">
              <div style={{ marginBottom: 20 }}>
                <h4 style={{ color: 'var(--text-secondary)', fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.6, margin: '0 0 12px' }}>Customer</h4>
                <p style={{ margin: 0, color: 'var(--text-primary)', fontWeight: 600 }}>{selectedOrder.customer?.name}</p>
                <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: 13 }}>{selectedOrder.customer?.email}</p>
              </div>
              <div style={{ marginBottom: 20 }}>
                <h4 style={{ color: 'var(--text-secondary)', fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.6, margin: '0 0 12px' }}>Current Status</h4>
                <OrderStatusBadge status={selectedOrder.status} />
              </div>
              <div>
                <h4 style={{ color: 'var(--text-secondary)', fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.6, margin: '0 0 12px' }}>Update Status</h4>
